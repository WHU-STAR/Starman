...
guest_one_liner() {
    local sid="$1"
    printf '%s' "cd /tmp && export STARMAN_SMOKE_ID=${sid} && ( wget -qO /tmp/guest-smoke-run.sh http://${HOST_HTTP}:${PORT}/guest-smoke.sh || curl -fsSL http://${HOST_HTTP}:${PORT}/guest-smoke.sh -o /tmp/guest-smoke-run.sh ) && chmod +x /tmp/guest-smoke-run.sh && bash /tmp/guest-smoke-run.sh"
}
